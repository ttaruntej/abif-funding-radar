...
class DiscoveryLock {
    static acquire() {
        if (fs.existsSync(LOCK_FILE)) {
            ...
            if (ageMs < TWO_HOURS) {
                console.error(`⚠️  ALREADY RUNNING: Discovery lock active (Age: ${Math.round(ageMs / 60000)}m). Aborting.`);
                process.exit(1);
            } else {
                console.warn('⚠️  STALE LOCK: Previous run likely crashed. Overwriting lock.');
            }
        }
...
async function runDiscovery() {
    ...
    try {
        if (fs.existsSync(CHECKPOINT_FILE)) fs.unlinkSync(CHECKPOINT_FILE); // Start fresh

        const clusters = JSON.parse(fs.readFileSync(QUERIES_PATH, 'utf-8'));
        console.log(`🚀 Starting V2.0 Discovery Loop for ${clusters.length} clusters.`);
        ...
        console.log(`\n🏁 All cycles complete. Running Final AI Audit & Merge...`);
...
console.log('ABIF Discovery Agent V2.0 Logic Loaded.');
